...
function searchAirportCodes(from, to, cb) {
	...
	getAirportCode.callAirportApi(from, function(err, fromPort) {
		...
		if (fromPort.length > 1) {
			return cb(new Error('Please select from the airports below for "From": '+util.inspect(fromPort)));
		}
		...
		getAirportCode.callAirportApi(to, function(err, toPort) {
			...
			if(toPort.length > 1) {
				return cb(new Error('Please select from the airport below for "To": '+util.inspect(toPort)));
			}
...
FlightSearch.prototype.processFlights = function() {
  return function(req, res, next) {
    searchAirlines(function(err, airlines) {
      if (err) {
      	return res.status(500).send('cannot get airlines data'+util.inspect(err));
      }
      var flightsData = [];
